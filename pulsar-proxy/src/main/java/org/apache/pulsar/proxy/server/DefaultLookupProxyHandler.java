...
public class DefaultLookupProxyHandler implements LookupProxyHandler {
                            ...
                            (brokerUrl, ex) -> {
                                if (ex != null) {
                                    ServerError serverError = ex instanceof LookupException
                                        ...
                                        : getServerError(ex);
                                    writeAndFlush(Commands.newLookupErrorResponse(serverError, ex.getMessage(),
                                        clientRequestId));
                                } else {
                                    writeAndFlush(
    ...
    protected CompletableFuture<String> performLookup(long clientRequestId, String topic, String brokerServiceUrl,
        ...
        try {
            ...
            return CompletableFuture.failedFuture(new LookupException(ServerError.MetadataError, e.getMessage()));
        }
     ...
     *
     **/
    protected void handlePartitionMetadataResponse(CommandPartitionedTopicMetadata partitionMetadata,
    ...
    protected void performGetTopicsOfNamespace(long clientRequestId,
    ...
    protected InetSocketAddress getAddr(String brokerServiceUrl, long clientRequestId) {
        URI brokerURI;
        try {
            brokerURI = new URI(brokerServiceUrl);
        } catch (URISyntaxException e) {
    ...
    protected void writeAndFlush(ByteBuf cmd) {
        final ChannelHandlerContext ctx = proxyConnection.ctx();
        NettyChannelUtil.writeAndFlushWithVoidPromise(ctx, cmd);
    }

    private static final Logger log = LoggerFactory.getLogger(DefaultLookupProxyHandler.class);
}
