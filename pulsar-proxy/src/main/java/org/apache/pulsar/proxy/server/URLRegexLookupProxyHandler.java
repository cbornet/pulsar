...
public class URLRegexLookupProxyHandler extends DefaultLookupProxyHandler {
    ...
    public void initialize(ProxyService proxy, ProxyConnection proxyConnection) {
        ...
        this.replacement = proxy.getConfiguration().getProperties().getProperty("urlRegexLookupProxyHandlerReplacement");
    }
                                    ...
                                    if (ex != null) {
                                        ...
                                        writeAndFlush(
                                            Commands.newLookupResponse(brokerUrl, brokerUrl, true,
                                                CommandLookupTopicResponse.LookupType.Connect, clientRequestId, true /* this is coming from proxy */));
                                    }
                                });
                    } else {
                        performLookup(clientRequestId, lookup.getTopic(), serviceUrl, false, 10)
                            .whenComplete(
                                (brokerUrl, ex) -> {
                                    if (brokerUrl.matches(regex)) {
                                        ...
                                        writeAndFlush(
                                            Commands.newLookupErrorResponse(ServerError.ServiceNotReady, "Broker URL does not match lookup handler regex",
                                                clientRequestId));
                                    }
                                });
